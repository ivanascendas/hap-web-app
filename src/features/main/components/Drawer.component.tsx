  ...
  ({ anchor, open, onClose }: DrawerProps, ref) => {
    ...
    return (
      ...
      >
        <Box
          ref={ref}
          sx={{ overflow: "auto", marginTop: { md: "0px", lg: "4.8125rem" } }}
          className="drawer-container"
        >
          <Toolbar
            sx={{ height: "4.8125rem", display: { md: "flex", lg: "none" } }}
          >
            <img
              src={logo}
              className="logo"
              ...
              <ListItem key={s.incDept} disablePadding>
                <ListItemButton
                  selected={location.pathname.includes(
                    departmentsInfo[s.incDept].url,
                  )}
                  onClick={() => handleClick(departmentsInfo[s.incDept].url)}
                >
                  <ListItemIcon>{departmentsInfo[s.incDept].icon}</ListItemIcon>
          ...
          <Divider />
          <List>
            {settings.map(({ icon, title, slug }) => (
              <ListItem key={title} disablePadding>
                <ListItemButton
                  onClick={() => handleClick(`/${slug}`)}
                  selected={location.pathname.includes(`/${slug}`)}
                >
