import { Box, CircularProgress, FormControl, InputLabel, MenuItem, Modal, Select, Skeleton, TablePagination } from "@mui/material";
import React, { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";
import '../Statement.component.scss';
import { useLazyGetPropertiesQuery, useLazyGetStatementsQuery } from "../../../shared/services/Statements.service";
import moment from "moment";
import currency from "../../../shared/utils/currency";
import { useAuth } from "../../../shared/providers/Auth.provider";
import { StatementDto, StatementQueryParams } from "../../../shared/dtos/statement.dtos";
...
export const RatesStatementComponent = ({ department, getBalance, balance, setStatementQueryParams }: RatesStatementProps): JSX.Element => {
    const [selectedProperty, setSelectedProperty] = useState("0");
    const [open, setOpen] = useState(false);
    const [selectedPeriod, setSelectedPeriod] = useState("current_year");
    const [selectedInvoice, setSelectedInvoice] = useState<InvoiceInputRequest | null>(null);
    const [page, setPage] = useState(0);
    const { t } = useTranslation();
    const { isAuthenticated } = useAuth();
    const [getProperties, { data: properties, isFetching }] = useLazyGetPropertiesQuery();
    const dispatch = useDispatch();
    const statements = useSelector(selectStatements);
    const statementCount = useSelector(selectStatementsCount);
    const [getStatements, { isFetching: isRatesLoading }] = useLazyGetStatementsQuery();
    ...
    return (<><Box className="personal_box ">
        <Box className="personal_box_filter">
            <Box>
                <FormControl fullWidth>
                    <InputLabel id="property-select-label" >{t('RATES.FILTER.PROPERTY')}</InputLabel>
                    {isFetching ? (
                    ...
                    ) : (properties && properties.length > 0 && <Select
                        labelId="property-select-label"
                        label={t('RATES.FILTER.PROPERTY')}
        ...
        <Box className="personal_box_content" sx={{ display: { xs: 'flex', md: 'none' } }}>
            <MobileStatementsListComponent onClick={rateClickHandler} list={statements || []} loadMore={handleLoadMore} />
        </Box>
        {selectedInvoice && <InvoicePopupСomponent dto={selectedInvoice} open={open} onClose={handleClose} />}
    </>);
};
