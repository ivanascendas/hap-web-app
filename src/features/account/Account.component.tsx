...
import { Box, FormControl, FormControlLabel, FormHelperText, FormLabel, Radio, RadioGroup, Skeleton, TextField, Typography } from "@mui/material";
...
import { useChangePasswordMutation, useCheckValidContactMutation, useSaveUserDataMutation } from "../../shared/services/Auth.service";
...
export const AccountComponent = (): JSX.Element => {
    ...
    const onSubmit = ({ EmailId, PhoneNumber, EmailConfirmed, PhoneNumberConfirmed, DefaultMFA }: ExsistingTenantDto) => {
        ...
        const model: ExsistingTenantDto = {
            ...
            PhoneExcludingCountryCode: PhoneNumber.replace(countryData?.dialCode || '353', ''),
            DefaultMFA,
            EmailConfirmed,
    ...
    const onPasswordSubmit = ({ oldPassword, newPassword, confirmPassword }: ChangePasswordDto) => {
    ...
    useEffect(() => {
        if (!isLoading && user?.defaultMFA) {
            ...
            const data: ExsistingTenantDto = {
                ...
                PhoneExcludingCountryCode: user?.phone?.replace(user?.phoneCountryCode || '353', '') || '',
            }
    ...
    console.log(formState, result, errors);
            ...
            <Box sx={{ flexGrow: 1 }} className="account" >
                ...
                <Box sx={{ display: { sm: 'block', md: 'none' } }}>
                    <Tabs value={tabValue} onChange={handleChange} aria-label="icon label tabs example">
                        <Tab icon={<UserIcon />} iconPosition="start" label={t('ACCOUNT.PERSONAL_DETAILS')} />
                        <Tab icon={<LockOpenIcon />} iconPosition="start" label={t('CONTENTS.NAV.CHANGE_PASSWORD')} />

                    </Tabs>
                </Box>
                <Box className="personal_box account_content">
                    <Box sx={{ display: { sm: tabValue == 0 ? 'block' : 'none', md: 'block' } }} className={`account_form  ${isLoading && 'loading'}`}>
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <Box className="account_form_row">
                                ...
                                <Box> <IntlTelInput
                                    ...
                                    {...register("PhoneNumber", {
                                        ...
                                        validate: (value) => {
                                            ...
                                            if (iniTelReff?.isValidNumber()) {
                                                console.log('valid phone number:', iniTelReff?.getNumber());
                                                return true;
                                            } else {
                                                console.log('invalid phone number:', iniTelReff?.getNumber());
                                    ...
                                    getIti={setIti}
                                />
                                    <FormHelperText error={!!errors.PhoneNumber}>{!!errors.PhoneNumber && t("ERRORS.INVALID_PHONE")}</FormHelperText>
                                </Box>
                                {isLoading && <Skeleton width={'100%'} height={'42px'} variant="rounded" />}
                            </Box>
                            <Box sx={{ display: { sm: 'block', md: 'none' } }} className="account_form_row MFA">
                                {!isLoading ? <FormControl >
                                    <FormLabel id="demo-radio-buttons-group-userdata-label">{t('MFA.INFO')}</FormLabel>
                                    <RadioGroup
                                        aria-labelledby="demo-radio-buttons-group-userdata-label"
                                        {...register("DefaultMFA")}

                                        value={watch('DefaultMFA')} // Add this line
                                        onChange={(e) => setValue('DefaultMFA', e.target.value, {
                                            shouldDirty: true,
                                            shouldTouch: true,
                                            shouldValidate: true,
                                        })} // Add this line
                                    ...
                                    >
                                        <FormControlLabel value="SMS" control={<Radio />} label={t('MFA.TYPES.SMS_OTP')} />
                                        <FormControlLabel value="Email" control={<Radio />} label={t('MFA.TYPES.EMAIL_OTP')} />
                                        <FormControlLabel value="None" control={<Radio />} label={t('MFA.TYPES.NO_OTP')} />
                                    </RadioGroup>
                                </FormControl> : <Skeleton width={'324px'} height={'200px'} variant="rounded" />}
                            </Box>
                            <Box className="account_form_row">
                                <button
                    ...
                    <Box sx={{ display: { sm: tabValue == 1 ? 'block' : 'none', md: 'block' } }} className={`account_form `}>
                        <form onSubmit={handleSubmitPasswords(onPasswordSubmit)}>
                            ...
                            <Box className="account_form_row">
                                ...
                                {!isLoading && user?.defaultMFA ? <TextField
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <Box sx={{ display: { sm: 'none', md: 'block' } }} className="account_form_row MFA">
                                {!isLoading ? <FormControl >
                                    <FormLabel id="demo-radio-buttons-group-label">{t('MFA.INFO')}</FormLabel>
                                    <RadioGroup
