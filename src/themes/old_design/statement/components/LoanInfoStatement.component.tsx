...
export const LoanInfoStatementComponent = ({
  department,
  ...
  const [getLoanInfo, { data: loanInfo, isFetching }] =
    useLazyGetLoanInfoQuery();

  const columns: ColumnItem<LoanInfoDto>[] = [
    {
      key: "loanNumber",
      label: "LOAN_INFO.COLUMNS.LOAN_NUMBER",
    },
    {
      key: "capitalBalance",
    ...
    { key: "interestRate", label: "LOAN_INFO.COLUMNS.INTEREST_RATE" },
    {
      ...
      rowRender: (row: LoanInfoDto) =>
        moment(row.loanEndDate).format("DD/MM/YYYY"),
    },
  ];
