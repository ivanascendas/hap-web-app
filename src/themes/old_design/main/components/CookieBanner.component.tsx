...
export const CookieBannerComponent = (): JSX.Element => {
  ...
  return showCookie ? (
    <div className="cookie-banner-container" role="banner">
      <div className="cookie-banner-content">
        <div className="cookie_info">
          <div className="h3">{t("COOKIE.POPUP.TITLE")}</div>
          <p
            className="coockie_info__p"
            dangerouslySetInnerHTML={{ __html: t("COOKIE.POPUP.CONTENT") }}
          />
        </div>
        <div className="cookie-banner-buttons-container">
          <button onClick={setCookie} className="cookie-banner-buttons ">
            {t("COOKIE.POPUP.GOT_IT")}
          </button>
        </div>
      </div>
    </div>
