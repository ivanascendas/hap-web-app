...
import { statementsApi } from "@shared/services/Statements.service";
...
const notifySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addMatcher(
      authApi.endpoints.saveUserData.matchFulfilled,
    ...
    builder.addMatcher(
      statementsApi.endpoints.downloadDocumentsPdf.matchFulfilled,
      (state: NotifyState, action: PayloadAction<any>) => {
...
export const selectNotify = (state: RootState): string | null =>
  state.notify.message;
export const selectNotifyDuration = (state: RootState): number =>
