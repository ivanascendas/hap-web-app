...
const statementsSlice = createSlice({
  ...
  reducers: {
    ...
    setStatements: (
      ...
      action: PayloadAction<StatementsState>,
    ) => {
    ...
    builder.addMatcher(
      ...
      (state, { payload, meta }) => {
        const { IncDept } = meta.arg
          .originalArgs as unknown as StatementQueryParams;
 ...
 */
export const selectStatements = (state: RootState): StatementDto[] =>
  state.statements.list;
export const selectStatementsCount = (state: RootState): number =>
  state.statements.count;
